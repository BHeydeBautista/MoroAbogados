...
const LawyerProfileClient = ({ slug }: { slug: string }) => {
  ...
  const recognitionImages = Array.from(
    new Set((lawyer.recognitionImages ?? []).filter(Boolean))
  );

  const recognitionCacheKey = lawyer.recognitionImagesCacheKey;
  const withCacheKey = (src: string) => {
    if (!recognitionCacheKey) return src;
