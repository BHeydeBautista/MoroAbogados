...
const AreaCard = ({ icon, title, points, link, index }: Props) => (
  <motion.div
    ...
    className="group/card bg-white/5 border border-white/10 rounded-xl p-6 shadow-lg transition-all duration-500 relative z-10 hover:z-20 hover:scale-[1.02] hover:brightness-110 hover:border-[#D4A75D]/30 min-h-[320px] flex flex-col justify-between"
  ...
  group-hover/card:blur-none group-hover/card:brightness-100"
    ...
    <div className="flex items-center gap-3 mb-3">
      <div className="w-10 h-10 flex items-center justify-center rounded-full bg-[#D4A75D]/10 text-[#D4A75D]">
        {icon}
      </div>
    ...
    <ul className="mt-4 space-y-2 text-sm text-white/80">
      ...
      {points.length > 3 && (
        <li className="text-[#D4A75D] text-xs uppercase tracking-wide mt-2">
          + {points.length - 3} más
        </li>
      )}
    </ul>
