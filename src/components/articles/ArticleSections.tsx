...
export default function ArticleSections({ sections = [], pdfUrl }: Props) {
  ...
  return (
    <div className="mt-8 space-y-6">
      {sections.map((s) => (
        <section key={s.id} id={s.id} className="bg-white/3 p-5 rounded-lg border border-white/8">
          <h4 className="text-lg font-semibold text-[#d4a75d]">{s.title}</h4>
          {s.summary && <p className="mt-2 text-white/80 text-sm">{s.summary}</p>}
          <div className="mt-3">
            <a
              href={pdfUrl ?? '#'}
              target="_blank"
